...
import { FormActionStateType, generalSettingErrorsType, generalSettingType, Step, VisibilityErrorsType, VisibilityType } from "@/types/types";
...
export default function Main() {
    ...
    const [setupData, setSetupData] = useState<generalSettingType>(stepOneFields)
        ...
        <>
            <form action={formAction} className="container-style mx-auto flex flex-col items-center justify-center transition-style">
                ...
                <div className={`w-full ${tab == "SETUP" ? "block" : "hidden"}`}>
                    <Setup errors={state?.step_one_errors as generalSettingErrorsType} setupData={setupData} setSetupData={setSetupData} />
                </div>
                <div className={`w-full ${tab == "VISIBILITY" ? "block" : "hidden"}`}>
                    <Visibility errors={state?.step_two_errors as VisibilityErrorsType} visibilityData={visibilityData} setVisibilityData={setVisibilityData} changeStepFn={changeStepFn} />
                </div>
