import { SocialType, UserSocialErrorType } from "@/actions/updatePersonalSocial";
import { ErrorMessageList } from "./ErrorMessageList";
import { SetStateAction } from "react";

export default function Links({ social, setSocial, errorMessages }: { social: SocialType; setSocial: (params: SetStateAction<SocialType>) => void; errorMessages: UserSocialErrorType }) {

  return (
    <>
      <div className="space-y-3 w-full">
        <label htmlFor="instagram" className="label-style">Instagram <span className="important-style">*</span></label>
          <div className="flex items-center">
            <div className="icon-style-secondary">
              <svg xmlns="http://www.w3.org/2000/svg" width="1em" height="1em" viewBox="0 0 24 24">
                <path d="M0 0h24v24H0z" fill="none" />
                <path fill="currentColor" fillRule="evenodd" d="M7.465 1.066C8.638 1.012 9.012 1 12 1s3.362.013 4.534.066s1.972.24 2.672.511c.733.277 1.398.71 1.948 1.27c.56.549.992 1.213 1.268 1.947c.272.7.458 1.5.512 2.67C22.988 8.639 23 9.013 23 12s-.013 3.362-.066 4.535c-.053 1.17-.24 1.97-.512 2.67a5.4 5.4 0 0 1-1.268 1.949c-.55.56-1.215.992-1.948 1.268c-.7.272-1.5.458-2.67.512c-1.174.054-1.548.066-4.536.066s-3.362-.013-4.535-.066c-1.17-.053-1.97-.24-2.67-.512a5.4 5.4 0 0 1-1.949-1.268a5.4 5.4 0 0 1-1.269-1.948c-.271-.7-.457-1.5-.511-2.67C1.012 15.361 1 14.987 1 12s.013-3.362.066-4.534s.24-1.972.511-2.672a5.4 5.4 0 0 1 1.27-1.948a5.4 5.4 0 0 1 1.947-1.269c.7-.271 1.5-.457 2.67-.511m8.98 1.98c-1.16-.053-1.508-.064-4.445-.064s-3.285.011-4.445.064c-1.073.049-1.655.228-2.043.379c-.513.2-.88.437-1.265.822a3.4 3.4 0 0 0-.822 1.265c-.151.388-.33.97-.379 2.043c-.053 1.16-.064 1.508-.064 4.445s.011 3.285.064 4.445c.049 1.073.228 1.655.379 2.043c.176.477.457.91.822 1.265c.355.365.788.646 1.265.822c.388.151.97.33 2.043.379c1.16.053 1.507.064 4.445.064s3.285-.011 4.445-.064c1.073-.049 1.655-.228 2.043-.379c.513-.2.88-.437 1.265-.822c.365-.355.646-.788.822-1.265c.151-.388.33-.97.379-2.043c.053-1.16.064-1.508.064-4.445s-.011-3.285-.064-4.445c-.049-1.073-.228-1.655-.379-2.043c-.2-.513-.437-.88-.822-1.265a3.4 3.4 0 0 0-1.265-.822c-.388-.151-.97-.33-2.043-.379m-5.85 12.345a3.669 3.669 0 0 0 4-5.986a3.67 3.67 0 1 0-4 5.986M8.002 8.002a5.654 5.654 0 1 1 7.996 7.996a5.654 5.654 0 0 1-7.996-7.996m10.906-.814a1.337 1.337 0 1 0-1.89-1.89a1.337 1.337 0 0 0 1.89 1.89" clipRule="evenodd" />
              </svg>
            </div>
            <input type="text" name="instagram_link" placeholder="e.g. https://web.instagram.com/example" className="input-style p-3! text-xs" value={social.instagram_link} onChange={(e) => setSocial(prevSocial => ({...prevSocial, instagram_link: e.target.value}))} />
          </div>
          <ErrorMessageList inputName="instagram_link" messages={errorMessages.instagram_link} />
      </div>
      <div className="space-y-3 w-full">
        <label htmlFor="facebook" className="label-style">Facebook <span className="important-style">*</span></label>
          <div className="flex items-center">
            <div className="icon-style-secondary">
              <svg xmlns="http://www.w3.org/2000/svg" width="1em" height="1em" viewBox="0 0 24 24">
                <path d="M0 0h24v24H0z" fill="none" />
                <g fill="none">
                  <g clipPath="url(#SVGXv8lpc2Y)">
                    <path fill="currentColor" fillRule="evenodd" d="M0 12.067C0 18.034 4.333 22.994 10 24v-8.667H7V12h3V9.333c0-3 1.933-4.666 4.667-4.666c.866 0 1.8.133 2.666.266V8H15.8c-1.467 0-1.8.733-1.8 1.667V12h3.2l-.533 3.333H14V24c5.667-1.006 10-5.966 10-11.933C24 5.43 18.6 0 12 0S0 5.43 0 12.067" clipRule="evenodd" />
                  </g>
                  <defs>
                    <clipPath id="SVGXv8lpc2Y">
                      <path fill="#fff" d="M0 0h24v24H0z" />
                    </clipPath>
                  </defs>
                </g>
              </svg>
            </div>
            <input type="text" name="facebook_link" placeholder="e.g. https://web.facebook.com/example" className="input-style p-3! text-xs" value={social.facebook_link} onChange={(e) => setSocial(prevSocial => ({...prevSocial, facebook_link: e.target.value}))} />
          </div>
          <ErrorMessageList inputName="facebook_link" messages={errorMessages.facebook_link} />
      </div>
      <div className="space-y-3 w-full">
          <label htmlFor="twitter_link" className="label-style">Twitter <span className="important-style">*</span></label>
          <div className="flex items-center">
            <div className="icon-style-secondary">
              <svg xmlns="http://www.w3.org/2000/svg" width="1em" height="1em" viewBox="0 0 24 24">
                <path d="M0 0h24v24H0z" fill="none" />
                <path fill="currentColor" d="M13.68 10.62L20.24 3h-1.55L13 9.62L8.45 3H3.19l6.88 10.01L3.19 21h1.55l6.01-6.99l4.8 6.99h5.24l-7.13-10.38Zm-2.13 2.47l-.7-1l-5.54-7.93H7.7l4.47 6.4l.7 1l5.82 8.32H16.3z" />
              </svg>
            </div>
            <input type="text" name="twitter_link" placeholder="e.g. https://x.com/example" className="input-style p-3! text-xs" value={social.twitter_link} onChange={(e) => setSocial(prevSocial => ({...prevSocial, twitter_link: e.target.value}))}/>
          </div>
          <ErrorMessageList inputName="twitter_link" messages={errorMessages.twitter_link} />
      </div>
      <div className="space-y-3">
          <label htmlFor="website" className="label-style">Website <span className="important-style">*</span></label>
          <div className="flex items-center">
            <div className="icon-style-secondary" >
              <svg xmlns="http://www.w3.org/2000/svg" width="1em" height="1em" viewBox="0 0 24 24">
                <path d="M0 0h24v24H0z" fill="none" />
                <path fill="currentColor" d="M16.36 14c.08-.66.14-1.32.14-2s-.06-1.34-.14-2h3.38c.16.64.26 1.31.26 2s-.1 1.36-.26 2m-5.15 5.56c.6-1.11 1.06-2.31 1.38-3.56h2.95a8.03 8.03 0 0 1-4.33 3.56M14.34 14H9.66c-.1-.66-.16-1.32-.16-2s.06-1.35.16-2h4.68c.09.65.16 1.32.16 2s-.07 1.34-.16 2M12 19.96c-.83-1.2-1.5-2.53-1.91-3.96h3.82c-.41 1.43-1.08 2.76-1.91 3.96M8 8H5.08A7.92 7.92 0 0 1 9.4 4.44C8.8 5.55 8.35 6.75 8 8m-2.92 8H8c.35 1.25.8 2.45 1.4 3.56A8 8 0 0 1 5.08 16m-.82-2C4.1 13.36 4 12.69 4 12s.1-1.36.26-2h3.38c-.08.66-.14 1.32-.14 2s.06 1.34.14 2M12 4.03c.83 1.2 1.5 2.54 1.91 3.97h-3.82c.41-1.43 1.08-2.77 1.91-3.97M18.92 8h-2.95a15.7 15.7 0 0 0-1.38-3.56c1.84.63 3.37 1.9 4.33 3.56M12 2C6.47 2 2 6.5 2 12a10 10 0 0 0 10 10a10 10 0 0 0 10-10A10 10 0 0 0 12 2" />
              </svg>
            </div>
            <input type="text" name="website_link" placeholder="e.g. https://example.com" className="input-style p-3! text-xs" value={social.website_link} onChange={(e) => setSocial(prevSocial => ({...prevSocial, website_link: e.target.value}))}/>
          </div>
          <ErrorMessageList inputName="website" messages={errorMessages.website_link} />
      </div>
    </>
  )
}